...
export class EmpAddEditComponent implements OnInit {
  ...
  onFormSubmit() {
    if (this.reqForm.valid) {
      ...
      const statusDescription = this.reqForm.get('StatusDescription')?.value;
      ...
      if (
       ...
       (selectedStatus !== 'R' || statusDescription !== '')
        ...
        if (this.data) {
          this._reqService.updateRequest(this.data.RequestID, this.reqForm.value).subscribe({
            next: (val: any ) => {

              this._coreService.openSnackBar('Requisição atualizada com sucesso!');
          ...
          this._reqService.addRequest(this.reqForm.value).subscribe({
            next: (val: any ) => {
              this._coreService.openSnackBar('Requisição criada com sucesso!');
              this._dialogRef.close(true);
            },
            error: (err: any) => {
              console.log(err);
        ...
        this._coreService.openSnackBar('Descrição não pode ser vazia para tipo Reprovado!');
      } else {
