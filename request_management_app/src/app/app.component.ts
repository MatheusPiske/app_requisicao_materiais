...
import { RequestService } from './emp-add-edit/services/request.service';
...
export class AppComponent implements OnInit {
  ...
  applyFilter(event: Event) {
    ...
    this.dataSource.filterPredicate = (data: any, filter: string) => {
      ...
      return requesterName.includes(filter) || itemDescription.includes(filter);
    };
  ...
  statusFilter: string = ''; // Initialize with no filter
  applyStatusFilter() {
    this.dataSource.filterPredicate = (data: any, filter: string) => {
      if (filter && filter.toLowerCase() !== 't') {
        return data.RequestStatus.toLowerCase().indexOf(filter.toLowerCase()) !== -1;
      } else {
        return true;
      }
    };
  ...
  formatPrice(price: number) {
    return this._currencyPipe.transform(price, 'BRL', 'symbol-narrow', '1.2-2');
  }
